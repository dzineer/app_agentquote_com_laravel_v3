...
class SiwlQuoter {
	...
	public function filterResults( $user, $results ) {
		...
		$carriers = CategoriesInsurance::getCarriers($user->id,4);

		$carrierIds = array();
		$carriersLookup = array();

        $ad_record = AffiliateAd::where('affiliate_id', '=', $user->affiliate_id)
            ->where('category_id', '=', self::SIWL_CATEGORY)
            ->first();
        ...
        if ($ad_record && $ad_record->company_id !== 0) {
            ...
            if (! $found) {
               ...
               // dd($carrier);
                die("carrier requested was not found");
        ...
        AQLog::quote('SIWLQuoter results: ' .  json_encode($results));
		...
		// $filteredPolicies = array_reverse($filteredPolicies);
	...
	public function getQuote($user, $fields) {
		...
		$fields['id'] = 3;
		$fields['period'] = 1;
		$fields['category'] = 'Final Expense (SIWL)';
//        $fields['state'] = 'MI';
        ...
        // AQLog::quote('Map fields: ' . json_encode($mappedFields));

		$quote = $gw->getQuote( $mappedFields );
		// echo "<br>quote: " . print_r($quote,true);

        // AQLog::info('Quote data return: ' . json_encode($quote));
