...
class ManagersController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        if ($request->has('sortby') && $request->has('order')) {
            ...
            if ( in_array($sortBy,  $sorts) &&
                ...
                $managers_with_pagination = $this->getAffiliateManagersWithPagination($affiliate, $sortBy, $request->input('order'));
            } else {
                $managers_with_pagination = $this->getAffiliateManagersWithPagination($affiliate);
            }
        } else {
            $managers_with_pagination = $this->getAffiliateManagersWithPagination($affiliate);
        }

        $pages = $this->genPages($managers_with_pagination['pagination']);

        $groups = AffiliateGroup::where('affiliate_id', '=', $affiliate_id)->get()->toArray();

        $pagination = $managers_with_pagination['pagination'];
        ...
        // return response()->json(  ["user" => $user, "managers" => json_encode($managers_with_pagination['managers']), "pagination" => json_encode($pagination), "groups" => json_encode($groups), "affiliate_id" => $affiliate_id]  );

        return view('managers.managers-list', ["user" => $user, "managers" => json_encode($managers_with_pagination['managers']), "pagination" => json_encode($pagination), "groups" => json_encode($groups), "affiliate_id" => $affiliate_id] );
    }
    ...
    private function getAffiliateManagersWithPagination($affiliate, $sortBy='id', $order = 'asc')
    {
        // \DB::enableQueryLog();
        $managers = [];
        ...
        if ($sortBy === 'id') {
            ...
            $managers = \DB::table('users')
                ...
                ->where('users.type_id', '=', self::MANAGER_USER)
        ...
        $users_with_pagination = json_decode(json_encode($managers));
        ...
        return ["managers" => $users, "pagination" => $users_with_pagination ];
    }
}
