...
[![Latest Version on Packagist](https://img.shields.io/packagist/v/dzineer/Laravel-AdminLTE.svg?style=flat-square)](https://packagist.org/packages/dzineer/Laravel-AdminLTE)
[![Build Status](https://travis-ci.org/dzineer/Laravel-AdminLTE.svg?branch=master)](https://travis-ci.org/dzineer/Laravel-AdminLTE)
[![Quality Score](https://img.shields.io/scrutinizer/g/dzineer/Laravel-AdminLTE.svg?style=flat-square)](https://scrutinizer-ci.com/g/dzineer/Laravel-AdminLTE)
[![StyleCI](https://styleci.io/repos/38200433/shield?branch=master)](https://styleci.io/repos/38200433)
[![Total Downloads](https://img.shields.io/packagist/dt/dzineer/Laravel-AdminLTE.svg?style=flat-square)](https://packagist.org/packages/dzineer/Laravel-AdminLTE)
...
1. Require the package using composer:
    ...
    ```
    composer require dzineer/laravel-adminlte
    ```
...
2. Add the service provider to the `providers` in `config/app.php`:
    ...
    ```php
    Dzineer\LaravelAdminLte\ServiceProvider::class,
    ```

3. Publish the public assets:

    ```
    php artisan vendor:publish --provider="Dzineer\LaravelAdminLte\ServiceProvider" --tag=assets
    ```
...
1. To update this package, first update the composer package:
    ...
    ```
    composer update dzineer/laravel-adminlte
    ```

2. Then, publish the public assets with the `--force` flag to overwrite existing files

    ```
    php artisan vendor:publish --provider="Dzineer\LaravelAdminLte\ServiceProvider" --tag=assets --force
    ```
...
```
php artisan vendor:publish --provider="Dzineer\LaravelAdminLte\ServiceProvider" --tag=config
```
...
use Dzineer\LaravelAdminLte\Menu\Filters\FilterInterface;
use Laratrust;
...
```php
'filters' => [
    Dzineer\LaravelAdminLte\Menu\Filters\ActiveFilter::class,
    Dzineer\LaravelAdminLte\Menu\Filters\HrefFilter::class,
    Dzineer\LaravelAdminLte\Menu\Filters\SubmenuFilter::class,
    Dzineer\LaravelAdminLte\Menu\Filters\ClassesFilter::class,
    //Dzineer\LaravelAdminLte\Menu\Filters\GateFilter::class, Comment this line out
...
```
php artisan vendor:publish --provider="Dzineer\LaravelAdminLte\ServiceProvider" --tag=translations
```
...
```
php artisan vendor:publish --provider="Dzineer\LaravelAdminLte\ServiceProvider" --tag=views
```
...
You can report issues and ask questions in the [issues section](https://github.com/dzineer/Laravel-AdminLTE/issues). Please start your issue with `ISSUE: ` and your question with `QUESTION: `
