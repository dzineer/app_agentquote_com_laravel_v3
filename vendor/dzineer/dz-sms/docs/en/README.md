...
First, add the DZ SMS package to your `require` in your `composer/json` file:
...
Add `Dzineer\SMS\SMSServiceProvider::class` in your `config/app.php` configuration file within the `providers` array.

#### Aliases

Finally, register the Facade.

Add `'SMS' => Dzineer\SMS\Facades\SMS::class` in your `config/app.php` configuration file within the `aliases` array.
...
DZ SMS operates in much of the same way as the Laravel Mail service provider.  If you are familiar with this then SMS should feel like home.  The most basic way to send a SMS is to use the following:
...
DZ SMS supports push SMS messages.  You must first configure this with your service provider by following the configuration settings above.
