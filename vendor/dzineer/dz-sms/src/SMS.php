...
use Dzineer\SMS\Drivers\DriverInterface;
...
class SMS
{
     ...
     *
     * @var \Dzineer\SMS\Drivers\DriverInterface
     */
    protected $driver;
     ...
     *
     * @return \Dzineer\SMS\OutgoingMessage The outgoing message that was sent.
     */
    public function send($view, $data, $callback)
    {
        \Illuminate\Support\Facades\Log::info( self::class . "::send" );

        $data['message'] = $message = $this->createOutgoingMessage();
        \Illuminate\Support\Facades\Log::info( self::class . "::send " . json_encode($data)  );
        // exit;
     ...
     *
     * @return \Dzineer\SMS\OutgoingMessage
     */
