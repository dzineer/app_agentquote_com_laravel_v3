...
const SortableItemCopy = SortableElement( ( {value, sortIndex} ) => {
    ...
    return <li className="list-group-item" style={ { "cursor": "pointer" } }>
        ...
        <a
        href={ '/user/modules?module=user_pages_module&page_id=' + page_id + '&section_id=' + section_id }
...
class UserPageSectionsListItems extends Component {
    constructor(props) {
        super(props);
        this.state = {
            items: null
    ...
    updatePageSectionPosition = (user_id, page_id, section_id, old_position, new_position) => {
    ...
    onSortEnd = ({oldIndex, newIndex}) => {
      ...
      this.setState( ({items}) => ({
          ...
          debugger;
          that.updatePageSectionPosition( that.state.user_id, draggedItem.page_id, draggedItem.section_id, oldIndex, newIndex );
      });
    };
...
UserPageSectionsListItems.propTypes = {
    items: PropTypes.array.isRequired,
    user_id: PropTypes.number.isRequired,
};

UserPageSectionsListItems.defaultProps = {
    items: [],
    user_id: 0
};

export default UserPageSectionsListItems;

if (document.getElementById('user-page-sections-list')) {
    render(
        <UserPageSectionsListItems items={ sections } user_id={ user_id } />,
        document.getElementById('user-page-sections-list')
    );
}
